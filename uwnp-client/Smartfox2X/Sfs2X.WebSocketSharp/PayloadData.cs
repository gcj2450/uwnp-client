...
namespace Sfs2X.WebSocketSharp
{
	internal class PayloadData : IEnumerable<byte>, IEnumerable
	{
		...
		/// <remarks>
		///   <para>
		///   A <see cref="T:Sfs2X.WebSocketSharp.WebSocketException" /> will occur if the payload data length is
		...
		public byte[] ApplicationData
		{
			get
			{
				return (_extDataLength > 0) ? _data.SubArray(_extDataLength, _length - _extDataLength) : _data;
			}
		}
		...
		public byte[] ExtensionData
		{
			get
			{
				return (_extDataLength > 0) ? _data.SubArray(0L, _extDataLength) : WebSocket.EmptyBytes;
			}
		}
