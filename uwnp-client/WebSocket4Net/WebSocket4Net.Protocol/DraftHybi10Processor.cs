...
namespace WebSocket4Net.Protocol
{
	internal class DraftHybi10Processor : ProtocolProcessorBase
	{
		...
		public override void SendHandshake(WebSocket websocket)
		{
			...
			stringBuilder.Append(string.Format("{0}: ", m_OriginHeaderName));
		...
		private byte[] EncodeDataFrame(int opCode, bool isFinal, byte[] playloadData, int offset, int length)
		{
			...
			if (isFinal)
			{
				array[0] = (byte)((uint)opCode | 0x80u);
			}
			else
			{
				array[0] = (byte)opCode;
			}
			array[1] = (byte)(array[1] | 0x80u);
			byte[] array2 = array;
			GenerateMask(array2, array2.Length - num - length);
			if (length > 0)
			{
				byte[] array3 = array;
				int outputOffset = array3.Length - length;
				byte[] array4 = array;
				MaskData(playloadData, offset, length, array3, outputOffset, array4, array4.Length - num - length);
			}
			return array;
		}
