	...
	public class SystemController : BaseController
	{
		...
		public override void HandleMessage(IMessage message)
		{
			if (sfs.Debug)
			{
				...
				array[0] = "Message: " + ((RequestType)message.Id).ToString() + " " + ((message != null) ? message.ToString() : null);
				logger.Info(array);
			}
		...
		private void FnUserExitRoom(IMessage msg)
		{
			...
			else
			{
				Logger logger = log;
				string[] array = new string[1];
				array[0] = "Failed to handle UserExit event. Room: " + ((roomById != null) ? roomById.ToString() : null) + ", User: " + ((userById != null) ? userById.ToString() : null);
				logger.Debug(array);
			}
		}
		...
		private void FnSpectatorToPlayer(IMessage msg)
		{
			...
			if (content.IsNull(BaseRequest.KEY_ERROR_CODE))
			{
				...
				if (roomById != null)
				{
					if (userById != null)
					{
						...
						Logger logger = log;
						string[] array = new string[1];
						string[] obj = new string[5] { "User: ", null, null, null, null };
						obj[1] = ((userById != null) ? userById.ToString() : null);
						obj[2] = " not joined in Room: ";
						obj[3] = ((roomById != null) ? roomById.ToString() : null);
						obj[4] = ", SpectatorToPlayer failed.";
			...
			else
			{
				...
				Logger logger2 = sfs.Log;
				object[] utfStringArray = content.GetUtfStringArray(BaseRequest.KEY_ERROR_PARAMS);
				string errorMessage = SFSErrorCodes.GetErrorMessage(short2, logger2, utfStringArray);
				dictionary["errorMessage"] = errorMessage;
		...
		private void FnPlayerToSpectator(IMessage msg)
		{
			...
			if (content.IsNull(BaseRequest.KEY_ERROR_CODE))
			{
				...
				if (roomById != null)
				{
					if (userById != null)
					{
						...
						obj[2] = " not joined in Room: ";
						obj[3] = ((roomById != null) ? roomById.ToString() : null);
						obj[4] = ", PlayerToSpectator failed.";
						array[0] = string.Concat(obj);
						logger.Warn(array);
					}
					else
			...
			else
			{
				...
				Logger logger2 = sfs.Log;
				object[] utfStringArray = content.GetUtfStringArray(BaseRequest.KEY_ERROR_PARAMS);
				string errorMessage = SFSErrorCodes.GetErrorMessage(@short, logger2, utfStringArray);
				dictionary["errorMessage"] = errorMessage;
				dictionary["errorCode"] = @short;
		...
		private void FnInitBuddyList(IMessage msg)
		{
			...
			else
			{
				...
				string errorMessage = SFSErrorCodes.GetErrorMessage(@short, logger, utfStringArray2);
				dictionary["errorMessage"] = errorMessage;
				dictionary["errorCode"] = @short;
